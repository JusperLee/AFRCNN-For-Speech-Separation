...
class U_Net_Block(nn.Module):
    ...
    def __init__(self,
        ...
        for i in range(states-1):
            self.latlayer.append(ConvNormAct(
                in_channels, in_channels, kernel_size=5, stride=1, groups=in_channels, mode='CN'))
        # Top-down skip connection
        self.top_down_skip = nn.Sequential(
            ConvNormAct(in_channels*(states-1), in_channels, 1, 1)
        )
        # Resual Path
    ...
    def forward(self, x):
        ...
        for i in range(len(top_down_f)-1):
...
class Recurrent(nn.Module):
    def __init__(self,
        ...
        self.mode = mode
        if mode == 'DC':
            self.msrnn = U_Net_Block(out_channels, in_channels, states)
        elif mode == 'CC':
            self.msrnn = U_Net_Block(out_channels, in_channels, states)
