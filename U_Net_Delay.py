...
class Bottomup(nn.Module):
    def __init__(self, in_channels=512, upsampling_depth=4):
        super().__init__()
        self.upsampling_depth = upsampling_depth
        self.downsample = nn.ModuleList([])
        for index in range(1, upsampling_depth):
            self.downsample.append(nn.Sequential(*[ConvNormAct(in_channels, in_channels,
                                                      kernel_size=5,
                                                      stride=2,
                                                      groups=in_channels, d=1) for i in range(index)]))
    def forward(self, x):
        output = []
        for i in range(self.upsampling_depth):
            if i == 0:
                ...
                output.append(self.downsample[i-1](x))
        return output

class U_Net_Delay_Block(nn.Module):
    '''
    ...
    def __init__(self,
        ...
        '''
        super(U_Net_Delay_Block, self).__init__()
        self.first = ConvNormAct(out_channels, in_channels, kernel_size=1,
                                 stride=1, groups=1)
        self.states = states
        # Bottom-up skip connection
        self.bottom_up_skip = Bottomup(in_channels, states)
        # Bottom-up
        self.bottom_up = nn.ModuleList([])
        self.bottom_up_con = nn.ModuleList([])
        self.bottom_up.append(ConvNormAct(
            in_channels, in_channels, kernel_size=5, stride=1, groups=in_channels, mode='CN'))
        for i in range(1, states):
            self.bottom_up_con.append(ConvNormAct(in_channels*2, in_channels, 1, 1))
            self.bottom_up.append(ConvNormAct(
                in_channels, in_channels, kernel_size=5, stride=2, groups=in_channels, mode='CN'))
        # Lateral connection
        self.latlayer = nn.ModuleList([])
        for i in range(states-1):
            self.latlayer.append(nn.Conv1d(in_channels, in_channels, 1, 1, padding=0))
        # Top-down
        if states > 1:
    ...
    def forward(self, x):
        ...
        bottomup_f = self.bottom_up_skip(output1)
        # Bottom-up
        output = [self.bottom_up[0](output1)]
        for k in range(1, self.states):
            out_k = self.bottom_up_con[k-1](torch.cat((self.bottom_up[k](output[-1]), bottomup_f[k]), dim=1))
            output.append(out_k)
        # Top-down
        top_down_f = []
        top_down_f.append(output[-1])
        for i in range(self.states-1):
            resampled_out_k = self.top_down(output.pop(-1))
            output[-1] = self.latlayer[i](output[-1]) + resampled_out_k
            top_down_f.append(output[-1])
        # Top-down skip connection
        wav_length = top_down_f[-1].shape[-1]
        for i in range(len(top_down_f)):
...
class Recurrent(nn.Module):
    def __init__(self,
        ...
        self.mode = mode
        if mode == 'DC':
            self.msrnn = U_Net_Delay_Block(out_channels, in_channels, states)
        elif mode == 'CC':
            self.msrnn = U_Net_Delay_Block(out_channels, in_channels, states)
...
class U_Net_Delay(nn.Module):
    def __init__(self,
                 ...
                 num_sources=2,
                 recurrent_mode='SC'):
        super(U_Net_Delay, self).__init__()
        '''
